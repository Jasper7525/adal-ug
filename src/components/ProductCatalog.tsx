...
export const ProductCatalog: React.FC<ProductCatalogProps> = ({ onContactDepot }) => {
  ...
  useEffect(() => { const onCatalogFilter = (event: Event) => { const filter = (event as CustomEvent<Filter>).detail; if (filter) setFilter(filter); }; window.addEventListener('adal-catalog-filter', onCatalogFilter); return () => window.removeEventListener('adal-catalog-filter', onCatalogFilter); }, []);
