...
import { Award, BadgeCheck, CheckCheck, Flame, Scale, Wrench } from 'lucide-react';
    ...
    <section id="features" className="py-16 bg-slate-50 border-y border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-100 text-cyan-800 text-xs font-bold uppercase tracking-wider mb-3"><Award className="w-3.5 h-3.5 text-cyan-600" /> Professional LPG Supply</div>
          <h2 className="text-2xl sm:text-4xl font-extrabold text-slate-900 tracking-tight font-display">Reliable Supply. Practical Safety. Professional Service.</h2>
          <p className="mt-3 text-base text-slate-600">Clear product information, dependable LPG supply and practical support for gas points, homes and commercial kitchens.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {trustPoints.map((point) => { const Icon = point.icon; return (
            <div key={point.title} className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
              <div><div className="flex items-center justify-between mb-4"><div className="w-12 h-12 rounded-xl bg-slate-900 text-white flex items-center justify-center shadow-md"><Icon className="w-6 h-6 text-orange-300" /></div><span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 border border-slate-200">{point.badge}</span></div><h3 className="text-lg font-bold text-slate-900 mb-2 font-display">{point.title}</h3><p className="text-sm text-slate-600 leading-relaxed">{point.description}</p></div>
              <div className="mt-5 pt-4 border-t border-slate-100 flex items-center gap-1.5 text-xs font-semibold text-cyan-800"><CheckCheck className="w-4 h-4 text-emerald-600" /><span>Adal service commitment</span></div>
            </div>
          ); })}
        </div>
      </div>
