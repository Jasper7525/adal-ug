...
app.use((req, _res, next) => {
  req.visitor = { method: req.method, path: req.path, ip: req.ip || req.headers['x-forwarded-for'] || 'unknown', userAgent: req.headers['user-agent'] || 'unknown', referrer: req.headers.referer || 'direct' };
  next();
});
...
app.use((req, res, next) => {
  res.on('finish', () => {
    ...
    const payload = { method: req.method, path: requestPath, ip: req.ip || req.headers['x-forwarded-for'] || 'unknown', user_agent: req.headers['user-agent'] || 'unknown', referrer: req.headers.referer || 'direct', status_code: res.statusCode, visited_at: new Date().toISOString() };
