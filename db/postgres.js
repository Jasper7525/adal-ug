...
export async function query(text, params = []) {
  if (!pool) throw new Error('PostgreSQL database is not configured. Set DATABASE_URL before starting the server.');
...
export async function initializeSchema() {
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS adal_products (
    id SERIAL PRIMARY KEY, code VARCHAR(100) UNIQUE NOT NULL, name VARCHAR(150) NOT NULL,
    size VARCHAR(80), category VARCHAR(80), price NUMERIC(10,2) NOT NULL DEFAULT 0,
    description TEXT, image_url VARCHAR(255), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`);

  await pool.query(`CREATE TABLE IF NOT EXISTS adal_product_images (
    id SERIAL PRIMARY KEY, product_code VARCHAR(100), category VARCHAR(80) DEFAULT 'cylinder',
    image_url VARCHAR(255) NOT NULL, image_name VARCHAR(150), mime_type VARCHAR(80), size_bytes INTEGER,
    price NUMERIC(10,2) DEFAULT 0, description TEXT, uploaded_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`);
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS adal_orders (
    id SERIAL PRIMARY KEY, customer_name VARCHAR(120), customer_phone VARCHAR(80), cylinder_code VARCHAR(100) NOT NULL,
    quantity INTEGER NOT NULL DEFAULT 1, delivery_zone VARCHAR(120), status VARCHAR(40) NOT NULL DEFAULT 'new',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`);

  await pool.query(`CREATE TABLE IF NOT EXISTS adal_visits (
  ...
  const defaults = [
    ...
    ['12.5KG','12.5kg Family Cylinder','12.5kg','12.5kg',90000,'Family LPG cylinder','/uploads/default-12.5kg.jpg'],
